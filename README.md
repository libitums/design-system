...
`npm run build`는 검증을 먼저 실행한 뒤 기본 token 116개를 `packages/design-tokens/dist/css/variables.css`, typography 변수 116개를 `packages/design-tokens/dist/css/typography.css`에 생성합니다. Foundation token 144개는 `packages/design-tokens/dist/index.js`와 `index.d.ts`로 생성합니다. 배포된 package에서는 다음 경로로 불러옵니다.
