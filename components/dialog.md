...
[Action Buttons](./action-buttons.md)를 **M 사이즈(40px), 가로 꽉 채움**으로 씁니다.
...
| 위치 | 역할 | 변형 |
|---|---|---|
| 위 | 사용자가 원래 하려던 것을 **계속**하는 쪽 | Solid `solid` |
| 아래 | 그만두거나 되돌리는 쪽 | Solid `subtle` |
...
- **접근성:** 열릴 때 포커스를 Dialog 안으로 옮기고 밖으로 나가지 않게 가둡니다. 닫히면 원래 위치로 되돌립니다. 배경 콘텐츠는 스크린리더에서 숨깁니다.
