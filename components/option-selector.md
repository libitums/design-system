...
| Size | 최소 높이 | 가로·세로 padding | Indicator | Label | 모서리 | 목록 간격 |
|---|---:|---|---|---|---|---|
| S | 48px | 12px `spacing.12` | 16px `icon.size.xs` | `typography.button.m` SemiBold 600, 14px / 20px / 0px | 12px `radius.md` | 8px `spacing.8` |
| M | 60px | 16px `spacing.16` | 20px `icon.size.sm` | `typography.button.xl` SemiBold 600, 16px / 24px / 0px | 16px `radius.lg` | 12px `spacing.12` |
| L | 68px | 20px `spacing.20` | 24px `icon.size.md` | `typography.heading.s` Bold 700, 18px / 24px / 0px | 16px `radius.lg` | 12px `spacing.12` |

- M은 기본값입니다.
- S는 짧은 단어를 고르거나 Grid로 선택지를 촘촘하게 배치할 때 사용합니다. 최소 높이 48px로 hit area 기준을 그대로 충족합니다.
- L은 이야기 장면의 응답 선택처럼 선택지가 적고 한 번의 선택을 강조해야 할 때 사용합니다.
- 텍스트 길이에 따라 Size를 자동으로 바꾸지 않습니다. 같은 흐름의 같은 목적에는 같은 Size를 사용합니다.

### Layout

| Layout | 값 | 토큰 |
|---|---|---|
| Stack | 1열, Option Item 사이는 Size의 목록 간격 | `spacing.8`, `spacing.12` |
| Grid | 같은 너비의 2열, 가로·세로 간격은 Size의 목록 간격, 같은 행의 높이는 가장 높은 Option Item에 맞춤 | `spacing.8`, `spacing.12` |
