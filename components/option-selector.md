...
1. **정답·오답 표시는 Feedback 축으로 추가합니다.** 제출 뒤 판정 결과를 보여줄 때는 Selection 값을 늘리지 않고 Option Item에 Feedback 축(None / Correct / Incorrect)을 더합니다. 색은 `feedback.correct-*`·`feedback.incorrect-*` 토큰에서 고르고, Indicator는 `8-ui/tick`·`8-ui/cross`로 구분하며, 판정 문구를 함께 제공해 색만으로 알리지 않습니다.
