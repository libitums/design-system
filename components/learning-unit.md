...
Learning Unit은 Ring, Surface, Icon만 포함합니다. 단위 제목, 진행 문구, 연결선은 상위 학습 목록이 조합하며 Learning Unit의 크기에 포함하지 않습니다.
...
- **학습 유형 아이콘은 접근성 트리에서 숨깁니다.** 학습 유형은 이름이 전달합니다.
...
2. **진행률 표시가 필요하면 Ring을 진행 바로 정의합니다.** Ring 두께와 지름을 그대로 두고 채움 각도와 남은 구간의 색을 이 문서에 함께 정합니다.
3. **새 Status는 Surface 색과 글리프를 함께 정합니다.** 색만 다른 Status를 늘리지 않고, 보조 기술에 전달할 state도 같이 정의합니다.
4. **새 크기는 바깥 지름·Ring 두께·Surface 지름을 함께 정합니다.** 위 산식으로 간격을 계산하고 hit area 48 × 48을 다시 확인합니다.
5. **보상·잠금 해제 연출은 Learning Unit이 소유하지 않습니다.** 장면 전체를 덮는 연출은 전용 컴포넌트로 정의합니다.
...
- **단위 제목은 `foundations/writing-tone.md`를 따릅니다.** 기능 이름보다 무엇을 배우는지 말하고 마침표를 붙이지 않습니다.
